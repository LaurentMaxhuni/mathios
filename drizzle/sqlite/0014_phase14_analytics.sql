...
CREATE TABLE IF NOT EXISTS learner_metrics (
  ...
  average_assessment_score REAL NOT NULL DEFAULT 0 CHECK (average_assessment_score >= 0 AND average_assessment_score <= 1),
  hints_used INTEGER NOT NULL DEFAULT 0 CHECK (hints_used >= 0),
  attempt_count INTEGER NOT NULL DEFAULT 0 CHECK (attempt_count >= 0),
  average_response_time_ms REAL NOT NULL DEFAULT 0 CHECK (average_response_time_ms >= 0),
  study_days INTEGER NOT NULL DEFAULT 0 CHECK (study_days >= 0),
  streak_days INTEGER NOT NULL DEFAULT 0 CHECK (streak_days >= 0),
  consistency_score REAL NOT NULL DEFAULT 0 CHECK (consistency_score >= 0 AND consistency_score <= 1),
...
CREATE TABLE IF NOT EXISTS content_metrics (
  ...
  accuracy REAL NOT NULL DEFAULT 0 CHECK (accuracy >= 0 AND accuracy <= 1),
  average_response_time_ms REAL NOT NULL DEFAULT 0 CHECK (average_response_time_ms >= 0),
  average_attempts REAL NOT NULL DEFAULT 0 CHECK (average_attempts >= 0),
  hint_rate REAL NOT NULL DEFAULT 0 CHECK (hint_rate >= 0),
  discrimination_index REAL NOT NULL DEFAULT 0 CHECK (discrimination_index >= -1 AND discrimination_index <= 1),
