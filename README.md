...
# Fill DATABASE_URL and NEON_AUTH_COOKIE_SECRET in .env.
npm run db:setup
npm run dev
```
...
The default `AUTH_MODE=neon-auth` opens the Neon Auth sign-in screen with email/password and Google authentication. After the first successful sign-in, Mathios creates the corresponding application profile; the first profile receives the learner and administrator roles. `db:seed` installs the three reference curricula, ten grade levels, five subjects, the complete Phase 20 domain and content catalog, mappings, curriculum-specific learning objectives, and an example classroom when profiles exist. For offline development, set `DATABASE_PROVIDER=sqlite`, use the local `DATABASE_URL`, and set `AUTH_MODE=local-profile`.

If the catalog is empty, confirm that the app and the setup command are using the same `.env` database, then run `npm.cmd run db:setup` once. The Playwright runner intentionally uses a temporary seeded SQLite database, so passing browser tests does not seed a developer's Postgres database. Authors can also open `/content-studio` to generate a structured lesson draft after enabling a local or remote AI provider; drafts remain unpublished until an author reviews them in the course and lesson editors.
...
Neon Postgres is the default database. The existing PostgreSQL adapter connects with the Neon-provided `DATABASE_URL`; checked-in migrations live under `drizzle/postgres`. SQLite remains an explicit offline fallback under `drizzle/sqlite`. Phase 1 adds the identity tables in `0001_phase1_identity.sql`; Phase 2 adds the educational structure in `0002_phase2_curriculum_structure.sql`; Phase 3 adds courses, modules, lessons, structured blocks, versions, and progress in `0003_phase3_courses_lessons.sql`; Phase 4 adds concepts and graph links in `0004_phase4_concepts_knowledge_graph.sql`; Phase 17 adds classroom membership, assignment, submission, rubric, feedback, and invitation tables in `0017_phase17_classrooms.sql`; Phase 18 adds append-only audit logs in `0018_phase18_deployment_hardening.sql`; `0019_neon_auth.sql` allows Neon Auth identities. Phase 20 is seed-only and reuses those tables. `npm run db:setup` applies the migrations and installs canonical roles, permissions, curriculum reference data, the complete Phase 20 scientific content catalog, idempotent course/content graph data, and profile-dependent classroom examples. See [the Phase 18 deployment notes](docs/phase18-deployment-hardening.md) for backups and SQLite-to-PostgreSQL migration.

`AUTH_MODE=neon-auth` uses the Neon Auth server SDK and UI. The app proxies Neon Auth through `/api/auth/*`, renders its flows at `/auth/sign-in` and related `/auth/*` paths, and enables Google through the Neon Auth UI provider. Account management is available under `/account/*`. `AUTH_MODE=local-profile` and `AUTH_MODE=local-credential` remain available for offline/local sessions; `AUTH_MODE=hosted` verifies configured HS256/RS256 bearer tokens.
...
The optional AI studio is available at `/ai`, and the author-facing content studio is available at `/content-studio`. AI is disabled by default, supports Ollama-compatible local models, OpenAI-compatible remote APIs, and hybrid local-first routing, and keeps provider keys server-side. Generated responses are grounded in bounded lesson, concept, grade, mastery, and learner context; they are labeled and reviewable without changing authoritative content. Content studio lesson drafts are validated against supported Mathios lesson blocks and saved as draft course content only after the author clicks save. See [the Phase 16 notes](docs/phase16-optional-ai.md).
