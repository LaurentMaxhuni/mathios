...
Phase 20 keeps content authoring inside the existing curriculum, courses, concepts, exercises,
assessments, and roadmaps modules. `src/domain/scientific-content/phase20.ts` owns the
framework-independent package contract and validates topic coverage, grade ranges, formula safety,
source attribution, and acyclic prerequisites. `src/infrastructure/database/phase20-content.ts`
owns deterministic reference data and derives rows for both database dialects; it does not import
React, Next.js, or database drivers. The canonical seed composes those arrays with the prior phase
seed records, and course prerequisites mirror the validated concept prerequisite graph.

Phase 20 introduces no migration or new route. Published lessons include accessible formula labels
and original SVG schematics with alt text, while the existing explorer, reader, exercise, assessment,
search, and roadmap surfaces remain the presentation boundary. Quality review of scientific accuracy,
local grade placement, attribution, and assessment difficulty remains a content-review responsibility
separate from software verification. See `docs/phase20-scientific-content-expansion.md` for the
catalog contract and verification checklist.
